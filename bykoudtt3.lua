...
-- The original ESP/render logic is kept below; only the menu UI is replaced
-- with the supplied Library UI.
-- =============================================================================
...
local Window = Library:CreateWindow({
    ...
    WatermarkText = "QA VISUALS",
    WatermarkLogo = "eye",
    SettingsTabEnabled = false
})

local SurvivorsPage = Window:AddTab("ESP Sobreviventes", "users")
local SurvivorsSection = SurvivorsPage:AddSection("Configurações", "settings")

local EspToggle = SurvivorsSection:AddToggle({
    Name = "Rastrear Jogadores",
    Flag = "EspPlayersEnabled",
    Default = true,
    Callback = function(Value)
...
local DistanceSlider = SurvivorsSection:AddSlider({
...
SurvivorsSection:AddLabel("Pressione [ K ] para ocultar/mostrar a interface")

local InfectedPage = Window:AddTab("ESP Infectados", "skull")
local InfectedSection = InfectedPage:AddSection("Em desenvolvimento", "bug")
InfectedSection:AddLabel("Nenhuma configuração disponível.")

local SuppliesPage = Window:AddTab("ESP Suprimentos", "package")
local SuppliesSection = SuppliesPage:AddSection("Em desenvolvimento", "package")
SuppliesSection:AddLabel("Nenhuma configuração disponível.")
    ...
    for i = 1, #playersList do
        ...
        if player ~= LocalPlayer then
            ...
            if character then
                ...
                if hrp and humanoid and hrp:IsA("BasePart") and humanoid:IsA("Humanoid") then
                    ...
                    if currentHealth > 0 then
                        ...
                        if distance <= MaxEspDistance then
                            ...
                            if onScreen and pos.Z > 0 then
                                ...
                                local factor = 45
                                local sizeX = (1000 / distance) * (factor / 45)
                                local sizeY = (1600 / distance) * (factor / 45)

                                sizeX = math.clamp(sizeX, 6, 120)
                                sizeY = math.clamp(sizeY, 10, 200)
                                ...
                                elements.Name.Position = UDim2.new(0, pos.X - 100, 0, posY - 28)
                                elements.Name.Size = UDim2.new(0, 200, 0, 14)
                                elements.Name.Visible = true

                                elements.Distance.Text = math.floor(distance) .. "m"
                                elements.Distance.Position = UDim2.new(0, pos.X - 100, 0, posY - 16)
                                elements.Distance.Size = UDim2.new(0, 200, 0, 14)
